				...
				<main id="main">
					...
					<div v-else>
						<div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-x-8 gap-y-16 grid-flow-dense"
							...
							<LazyTeamMember v-for="(member, index) in filtered" :key="member.id" :data-index="index"
								:selected="curruntMember" v-bind="member" @open="handleClick(member.id)" @close="handleClose" />
						</div>
					</div>
				</main>
			</div>
