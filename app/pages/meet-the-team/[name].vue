	...
	<div class="container mx-auto relative mb-12">
		<div
			...
			<div class="col-start-1 row-span-2 row-start-2 md:row-start-1">
				<NuxtImg provider="imagekit" :src="TeamMember?.srcImg" height="437" width="350"
					:alt="`A photo of ${TeamMember?.firstName} ${TeamMember?.lastName} `"
				...
				<ul class="flex flex-col mt-6">
					...
					<li class="listItem" v-if="TeamMember?.email">
						...
						<a :href="`mailto:${TeamMember?.email}`"><span class="font-bold">Email:</span> {{
							TeamMember?.email }}</a>
					</li>
				</ul>
			</div>
...
<UForm ref="form" :state="state" class="space-y-4" @submit="onSubmit">
	<UFormGroup label="Your Name" name="name" required>
		<UInput variant="outline" color="midnight" v-model="state.name" type="text" placeholder="Enter your name" />
	</UFormGroup>

	<UFormGroup label="Your Email" name="email" required>
		<UInput variant="outline" color="midnight" v-model="state.email" type="email" placeholder="Enter your email"
			icon="i-heroicons-envelope" />
	</UFormGroup>

	<UFormGroup label="Contact Number" name="phone" required>
		<UInput variant="outline" color="midnight" v-model="state.phone" type="text" />
	</UFormGroup>

	<UFormGroup label="Subject" name="subject" required>
		<UInput variant="outline" color="midnight" v-model="state.subject" type="text" />
	</UFormGroup>

	<UFormGroup label="Message" name="message" required>
		<UTextarea variant="outline" color="midnight" v-model="state.message" type="text" />
	</UFormGroup>
	...
	<UButton class="bg-midnight-950" color="midnight" variant="solid" type="submit">
		Submit
	</UButton>
</UForm>
</UCard>
