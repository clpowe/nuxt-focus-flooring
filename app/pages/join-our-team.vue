		...
		<div class="">
			...
			<main id="main" class="main pt-16">
				<article class="container space-y-6">
					<div>
						<h2 class="font-bold uppercase text-4xl lg:text-4xl text-gray-800 dark:text-gray-200 mb-4">
							<span
								class="underline decoration-[var(--focus-yellow)] decoration-4 underline-offset-8">Culture</span>
						</h2>
						<p class="max-w-lg">
							<strong>Illuminating our industry with positivity, partnership, &
								precision</strong>
							starts within. We look to be the light to our team so that
							together we can light up the industry.
						</p>
					</div>
					<div class="my-grid grid grid-cols-1 sm:grid-cols-2 gap-4 lg:grid-cols-3 xl:grid-cols-4">
						<div v-for="pic in communityPictures">
							<NuxtImg :src="pic.img" :alt="pic.alt" provider="imagekit" fit="cover" height="380"
								width="333" sizes="100vw sm:400px md:600px lg:800px xl:1400px" quality="90"
								format="webp" />
						</div>
					</div>
				</article>
				<article class="max-w-lg mx-auto">
					<h2 class="font-bold uppercase text-4xl lg:text-4xl text-gray-800 dark:text-gray-200 mb-4">
						<span
							class="underline decoration-[var(--focus-yellow)] decoration-4 underline-offset-8">Join</span>
						Our Team to Make a
						<span
							class="underline decoration-[var(--focus-yellow)] decoration-4 underline-offset-8">Difference</span>
					</h2>
					<div v-if="success">
					...
					<div v-else>
						...
						<UForm :schema="schema" :state="state" class="max-w-lg mx-auto" @submit="onSubmit">
							<UFormGroup label="First Name" name="firstName" v-slot="{ error }">
								<UInput padded size="md" color="gray" variant="outline" v-model="state.firstName" />
							</UFormGroup>
							<UFormGroup label="Last Name" name="lastName">
								<UInput padded size="md" color="gray" variant="outline" v-model="state.lastName" />
							</UFormGroup>
							<UFormGroup label="Email" name="email">
								<UInput padded size="md" color="gray" variant="outline" v-model="state.email" />
							</UFormGroup>
							<UFormGroup label="Phone Number" name="phone">
								<UInput padded size="md" color="gray" variant="outline" v-model="state.phone" />
							</UFormGroup>
							<UFormGroup label="How did you hear about us?" name="howDidYouHear">
								<UTextarea padded size="md" color="gray" variant="outline"
									v-model="state.howDidYouHear" />
							</UFormGroup>
							<UFormGroup label="What is your department of interest?" name="deptOfInterest">
								<USelectMenu padded size="md" aria-label="Select Department of Interest"
									v-model="state.deptOfInterest" :options="interest" />
							</UFormGroup>
							<UFormGroup label="Comments" name="comments">
								<UTextarea padded size="md" color="gray" variant="outline" v-model="state.comments" />
							</UFormGroup>
							<UButton variant="solid" type="submit" class="text-midnight-950" color="focusyellow">
								Submit
							</UButton>
						</UForm>
					</div>
